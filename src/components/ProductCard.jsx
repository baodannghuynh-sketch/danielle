// src/components/ProductCard.jsx
import { Link } from 'react-router-dom';
import { useState, useMemo } from 'react';
import { useCartStore } from '../store/useCartStore';

export default function ProductCard({ product }) {
  const [hovered, setHovered] = useState(false);
  const [imgIndex, setImgIndex] = useState(0);

  const addItem = useCartStore((state) => state.addItem);
  const toggleWishlist = useCartStore((state) => state.toggleWishlist);
  const wishlist = useCartStore((state) => state.wishlist);

  const [now] = useState(() => Date.now());

  // Ảnh
  const images = Array.isArray(product.image_url)
    ? product.image_url.filter(Boolean)
    : [product.image_url || product.image].filter(Boolean);

  const currentImage =
    images[imgIndex] ||
    'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?w=800&q=80';

  const isNew = useMemo(() => {
    if (product.is_new) return true;
    if (product.created_at) {
      return now - new Date(product.created_at) < 7 * 24 * 60 * 60 * 1000;
    }
    return false;
  }, [product.is_new, product.created_at, now]);

  const isSoldOut = product.in_stock === false;
  const isLimited = product.stock_quantity && product.stock_quantity <= 3;

  const isLiked = wishlist?.some((w) => w.id === product.id);

  return (
    <div
      style={{
        position: 'relative',
        borderRadius: '24px',
        overflow: 'hidden',
        background: '#fff',
        boxShadow: hovered
          ? '0 30px 70px rgba(165, 28, 48, 0.18)'
          : '0 10px 40px rgba(0,0,0,0.08)',
        transition: 'all 0.6s cubic-bezier(0.23, 1, 0.32, 1)',
        transform: hovered ? 'translateY(-16px) scale(1.02)' : 'translateY(0) scale(1)',
      }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => {
        setHovered(false);
        setImgIndex(0);
      }}
      onMouseMove={() => {
        if (images.length > 1 && hovered) {
          setImgIndex((prev) => (prev + 1) % images.length);
        }
      }}
    >
      {/* CLICK VÀO CARD => TRANG CHI TIẾT */}
      <Link
        to={`/product/${product.id}`}
        style={{ textDecoration: 'none', color: 'inherit' }}
      >
        {/* Ảnh + overlay */}
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          <img
            src={currentImage}
            alt={product.name}
            loading="lazy"
            style={{
              width: '100%',
              height: '400px',
              objectFit: 'cover',
              transition: 'transform 1.2s ease',
              transform: hovered ? 'scale(1.12)' : 'scale(1)',
            }}
            onError={(e) => {
              e.currentTarget.src =
                'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?w=800&q=80';
            }}
          />

          {/* Overlay đỏ rượu vang nhè nhẹ */}
          <div
            style={{
              position: 'absolute',
              inset: 0,
              background:
                'linear-gradient(135deg, rgba(165,28,48,0.15) 0%, rgba(165,28,48,0.35) 100%)',
              opacity: hovered ? 1 : 0,
              transition: 'opacity 0.6s ease',
            }}
          />

          {/* Badge NEW / LIMITED / SOLD OUT */}
          <div
            style={{
              position: 'absolute',
              top: '20px',
              left: '20px',
              display: 'flex',
              flexDirection: 'column',
              gap: '10px',
              zIndex: 10,
            }}
          >
            {isNew && <span style={badgeStyle('#A51C30')}>NEW</span>}
            {isLimited && !isSoldOut && (
              <span style={badgeStyle('#ff4444')}>
                Còn {product.stock_quantity}
              </span>
            )}
            {isSoldOut && <span style={badgeStyle('#000')}>HẾT HÀNG</span>}
          </div>

          {/* ❤️ ở góc phải trên ảnh */}
          <button
            onClick={(e) => {
              e.preventDefault();
              toggleWishlist(product);
            }}
            style={{
              position: 'absolute',
              top: '20px',
              right: '20px',
              width: '40px',
              height: '40px',
              borderRadius: '999px',
              border: 'none',
              background: 'rgba(0,0,0,0.55)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              cursor: 'pointer',
              zIndex: 11,
            }}
          >
            <span
              style={{
                fontSize: '20px',
                color: isLiked ? '#ff4b7d' : '#fff',
                textShadow: isLiked ? '0 0 10px rgba(255,75,125,0.7)' : 'none',
              }}
            >
              {isLiked ? '♥' : '♡'}
            </span>
          </button>
        </div>

        {/* Thông tin */}
        <div style={{ padding: '24px 22px 18px', textAlign: 'center' }}>
          {product.category && (
            <p
              style={{
                color: '#A51C30',
                fontSize: '12px',
                letterSpacing: '3px',
                textTransform: 'uppercase',
                margin: '0 0 8px',
                fontWeight: 600,
              }}
            >
              {product.category}
            </p>
          )}

          <h3
            style={{
              fontSize: '22px',
              margin: '0 0 10px',
              letterSpacing: '1.5px',
              color: '#111',
              fontFamily: '"Playfair Display", serif',
              fontWeight: 500,
            }}
          >
            {product.name}
          </h3>

          <p
            style={{
              color: '#A51C30',
              fontSize: '24px',
              fontWeight: 'bold',
              margin: '8px 0',
              letterSpacing: '1px',
            }}
          >
            {Number(product.price).toLocaleString('vi-VN')} ₫
          </p>

          {product.old_price && (
            <p
              style={{
                color: '#999',
                fontSize: '14px',
                textDecoration: 'line-through',
                margin: '0 0 8px',
              }}
            >
              {Number(product.old_price).toLocaleString('vi-VN')} ₫
            </p>
          )}
        </div>
      </Link>

      {/* 🛒 NÚT DƯỚI ẢNH – KHÔNG CẦN VÀO CHI TIẾT */}
      <div
        style={{
          padding: '0 22px 24px',
          display: 'flex',
          justifyContent: 'center',
        }}
      >
        <button
          onClick={() => addItem(product)}
          disabled={isSoldOut}
          style={{
            width: '100%',
            padding: '14px 20px',
            borderRadius: '999px',
            border: 'none',
            background: isSoldOut ? '#999' : '#A51C30',
            color: 'white',
            fontWeight: 'bold',
            fontSize: '13px',
            letterSpacing: '2px',
            cursor: isSoldOut ? 'not-allowed' : 'pointer',
            boxShadow: '0 10px 30px rgba(165,28,48,0.4)',
            textTransform: 'uppercase',
            transition: 'all 0.35s ease',
          }}
          onMouseEnter={(e) => {
            if (!isSoldOut) {
              e.currentTarget.style.background = '#c51c35';
              e.currentTarget.style.transform = 'translateY(-3px)';
            }
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = isSoldOut ? '#999' : '#A51C30';
            e.currentTarget.style.transform = 'translateY(0)';
          }}
        >
          {isSoldOut ? 'HẾT HÀNG' : 'THÊM VÀO GIỎ'}
        </button>
      </div>
    </div>
  );
}

const badgeStyle = (bg) => ({
  background: bg,
  color: 'white',
  padding: '8px 18px',
  borderRadius: '30px',
  fontSize: '11px',
  fontWeight: 'bold',
  letterSpacing: '2px',
  textTransform: 'uppercase',
  boxShadow: '0 4px 15px rgba(0,0,0,0.3)',
  display: 'inline-block',
});
